...
| 2.1/2.2/2.3 Complaints | `POST/GET /api/v1/complaints`, `PATCH /api/v1/complaints/{id}/resolution` | `POST/GET /api/v1/complaints/tickets` (also mounted at `/api/v1/user/tickets`); resolution is `PATCH /api/v1/manager/tickets/{id}`. The officer grid (`GET /api/v1/manager/dashboard`) now returns a server-side `is_aging` flag per complaint row (PR #127) and caps the complaint collection at 20 rows (PR #126); there is still no client-driven pagination (no page/offset parameter). |
