...
| RCY-07 | Authorization | Manager ward scoping | Unassigned manager assigns a batch | `403 Forbidden` | No batch mutation | `tests/api/features/recycler/test_batch_lifecycle.py::test_manager_without_managed_ward_cannot_assign_batch` | Passed. Unassigned manager denied with 403 Forbidden. | No assignment persisted. | Pass | Resolved in PR #108 |
...
| RCY-12 | Authorization | Recycler batch boundary | Recycler accepts batch assigned to another recycler | `403 Forbidden` | No state change | `tests/api/features/recycler/test_recycler_accept_other_batch` | Passed. Cross-recycler acceptance denied. | No mutation. | Pass | — |
| RCY-13 | Lifecycle | Recycler rejection | Recycler rejects assigned batch with note | `200 OK`; status reverts to `COLLECTED` | Batch unassigned, note saved, manager notified | `tests/api/features/recycler/test_recycler_reject_batch` | Passed. Rejection unassigned batch and notified manager. | Status reverted to `COLLECTED`, recycler cleared, note saved. | Pass | — |
| RCY-14 | Boundary | Rejection note validation | Recycler rejects batch with whitespace-only note | `422 Unprocessable Entity` | Batch remains `ASSIGNED` | `tests/api/features/recycler/test_recycler_reject_whitespace_only_note` | Passed. Whitespace note rejected with 422 Unprocessable Entity. | Batch remains ASSIGNED; no empty note saved. | Pass | Resolved in PR #109 |
...
Execution timestamp: 2026-08-12T21:05:00+00:00
...
|---|---|---|---|---|---|
| RCY-QA-01 | High | P0 | Unassigned municipal officer can assign batches outside any ward | RCY-07 | Fixed in PR #108 |
| RCY-QA-02 | Medium | P1 | Whitespace-only rejection notes are accepted and stored as empty after stripping | RCY-14 | Fixed in PR #109 |

## Root cause & resolution analysis

- **RCY-QA-01:** `assign_batch` in `backend/app/features/materials/service.py` was updated from `if zone_ids and batch.zone_id not in zone_ids:` to `if batch.zone_id not in zone_ids:`. Unassigned officers (`zone_ids = []`) now correctly trigger `403 Forbidden`. (Resolved in PR #108).
- **RCY-QA-02:** `RejectBatchRequest` schema in `backend/app/features/materials/schemas.py` was enhanced with `@field_validator("note")` to strip whitespace and reject empty notes with `422 Unprocessable Entity`. (Resolved in PR #109).

## Final status

```text
SCRUM-194 Recycler Dashboard & Gamification QA: PASSED (All defects resolved)
Focused suite: 84 passed, 0 failed (84 collected)
