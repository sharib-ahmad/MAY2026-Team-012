...
export default function EcoBotChat() {
  ...
  return (
    <div className="fade-in">
      {/* Page header */}
      <div className="mb-5">
        <p className="text-[10px] font-mono-civic uppercase tracking-widest text-gray-400">Concierge</p>
      ...
      <div className="grid lg:grid-cols-[2fr_1fr] gap-4 items-start">
        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-soft flex flex-col h-[65vh] min-h-[420px] overflow-hidden">
          <div className="flex-1 overflow-y-auto p-5 space-y-4">
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} rise-in`}>
                {m.role === 'bot' && (
            ...
            {typing && (
              <div className="flex items-center gap-2 rise-in">
                ...
                <div className="bg-[#F3EEE1] rounded-2xl rounded-bl-sm px-4 py-3 flex items-center gap-1.5">
                  ...
                  <span className="w-1.5 h-1.5 rounded-full bg-gray-300 animate-pulse-slow" style={{ animationDelay: '0.2s' }} />
                  <span className="w-1.5 h-1.5 rounded-full bg-gray-300 animate-pulse-slow" style={{ animationDelay: '0.4s' }} />
                </div>
              </div>
            )}
            <div ref={bottom} />
