  ...
  const handleFiles = (fileList) => {
    ...
    for (const f of incoming) {
      ...
      if (f.size > MAX_PHOTO_MB * 1024 * 1024) {
        setFieldErrors((prev) => ({
          ...prev,
          photos: `Each photo must be ${MAX_PHOTO_MB}MB or smaller.`,
        }));
        return;
      }
    }
    ...
    <div className="max-w-2xl mx-auto fade-in">
      ...
      <Card>
        <form onSubmit={submit} className="space-y-4" noValidate>
          {fieldErrors.submit && (
            <div className="bg-red-50 text-red-700 text-sm p-3 rounded-input">
              {fieldErrors.submit}
            </div>
          )}
          <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              {fieldErrors.condition && (
                <p className="text-xs text-red-600 mt-1">{fieldErrors.condition}</p>
              )}
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            />
            {fieldErrors.photos && (
              <p className="text-xs text-red-600 mt-1">{fieldErrors.photos}</p>
            )}
            {files.length > 0 && (
